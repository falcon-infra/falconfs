...
Datum falcon_batch_meta_call_by_shmem(PG_FUNCTION_ARGS)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        switch (operation_type) {
            ...
            case 11: /* DFC_READDIR */
                infoDataArray[i].path = ReadString(&p);
                infoDataArray[i].maxReadCount = *(int32_t*)p;
                p += 4;
                infoDataArray[i].lastShardIndex = *(int32_t*)p;
                p += 4;
                infoDataArray[i].lastFileName = ReadString(&p);
                break;

            case 15: /* DFC_MKDIR_SUB_MKDIR */
                infoDataArray[i].parentId = *(uint64_t*)p;
                p += 8;
                infoDataArray[i].path = ReadString(&p);  /* name */
                infoDataArray[i].st_ino = *(uint64_t*)p;  /* inode_id */
                p += 8;
                break;

            case 16: /* DFC_MKDIR_SUB_CREATE */
                infoDataArray[i].parentIdPartId = *(uint64_t*)p;
                p += 8;
                infoDataArray[i].path = ReadString(&p);  /* name */
                infoDataArray[i].st_ino = *(uint64_t*)p;  /* inode_id */
                p += 8;
            ...
            case 19: /* DFC_RENAME_SUB_RENAME_LOCALLY */
                infoDataArray[i].srcParentId = *(uint64_t*)p;
                p += 8;
                infoDataArray[i].srcParentIdPartId = *(uint64_t*)p;
                p += 8;
                infoDataArray[i].path = ReadString(&p);  /* src_name */
                infoDataArray[i].dstParentId = *(uint64_t*)p;
                p += 8;
                infoDataArray[i].dstParentIdPartId = *(uint64_t*)p;
                p += 8;
                infoDataArray[i].dstPath = ReadString(&p);  /* dst_name */
                infoDataArray[i].targetIsDirectory = *(uint8_t*)p;
                p += 1;
                infoDataArray[i].directoryInodeId = *(uint64_t*)p;
                p += 8;
            ...
            case 20: /* DFC_RENAME_SUB_CREATE */
                ...
                p += 8;
                infoDataArray[i].path = ReadString(&p);  /* name */
                infoDataArray[i].st_ino = *(uint64_t*)p;
                p += 8;
