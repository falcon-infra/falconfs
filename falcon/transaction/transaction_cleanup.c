...
void RemoveInprogressTransaction(const char *transactionName)
{
    ...
    if (!found)
        (WARNING, "%s, removing invalid transaction name.", FalconErrorCodeToString[PROGRAM_ERROR]);
...
static bool CommitOrRollbackPreparedTransaction(int serverId, char *transactionGid, bool isCommit)
{
    ...
    FALCON_ELOG_LOG_EXTENDED("cleanup a prepared transaction on server: %d, command: %s",
                             serverId,
                             command);
