...
/* Per-gap aggregation data (used in output worker, NOT in shmem) */
typedef struct GapStat
{
    uint64_t sum_ns;
    uint64_t count;
    uint64_t min_ns;
...
/* Per-opcode aggregation (used in output worker, NOT in shmem) */
typedef struct OpcodeGapStats
{
    uint64_t requestCount;
    uint64_t e2eSumNs;
    int maxCheckpointCount;
    GapStat gaps[STAT_MAX_CHECKPOINTS - 1];
} OpcodeGapStats;
...
{
    if (statIndex < 0 || g_FalconPerRequestStatShmem == NULL ||
